                    ...
                    <form class="cmxform" id="formAgregarNuevaAgencia" name="formAgregarNuevaAgencia"
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="txtTipoAgencia" name="txtTipoAgencia">
                                ...
                                <option value="2">Instalacion y Comercializadora</option>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form role="form">
                    <div class="form-group">
                        <input type hidden="" value ="" id="idAgenciaEliminar" />
...
<script type="text/javascript">
    ...
    function loadUsers() {
        ...
        var tipoUsuario=(localStorage.getItem("id"));
        var resTipUs = tipoUsuario.toUpperCase();
        //alert(rolUsuario);
        $.ajax({
            method: "POST",
            ...
            success: function (data) {
                ...
                for (i = 0; i < sizeAgenciasData; i++) {
                    ...
                    var botonesAccion = "";
                    if (rolUsuario != 'Agency' && nickname.toLowerCase()!='ayopsa' && nickname.toLowerCase()!='callcenter'  ) {
                        botonesAccion = "<button type='button' class='btnEditUser btn btn-success btn-xs' onClick='edit(" + id + ");'><i class='fa fa-pencil'></i></button>" +
                            "<button onClick='eliminar(" + id + ");' type='button' class='btnDeleteUser btn btn-danger btn-xs'><i class='fa fa-trash-o '></i></button>";
                    } else {

                    }
                    console.log('rol',rol);
                    if((rol == 'Instalacion' || rol == 'Instalacion y Comercializadora') &&
                        (resTipUs == 'SUPERADMIN')){
    ...
    $("#txtTipoAgencia").change(function(e){
        var idTipo = $(this).val();
        if(idTipo == 1){
            $("#txtPerfilAgencia").empty();
            $("#txtPerfilAgencia").append('<option value="4">Instalacion<option>');
        }else{
            $("#txtPerfilAgencia").empty();
            $("#txtPerfilAgencia").append('<option value="1">Cencista</option><option value="2">Venta</option><option value="3">Plomero</option><option value="4">Instalacion</option><option value="5">Censista &amp; Venta</option><option value="6">Plomero &amp; Venta</option><option value="7">Plomero &amp; Venta &amp; Censista</option><option value="8">Plomero &amp; Venta &amp; Censista &amp; Instalador</option>');
        }
    });
    ...
    {
        $("#idAgenciaEliminar").val(idAgencia);
    ...
    $('#btnCancelUser').click(function () {
        ...
        $.ajax({
            method: "POST",
            url: "dataLayer/callsWeb/EliminarAgencys.php",
            data:{
                "idUsuario":$("#idAgenciaEliminar").val()
            },
            dataType: "JSON",
            success: function (data) {
                ...
                {
                    $("#idAgenciaEliminar").val("");
                    $('#modalDelete').modal('hide');
                    redireccionarAlTerminarToast('agencies.php');
    ...
    function agenciesLoad() {
        $.ajax({
            method: "GET",
            url: "dataLayer/callsWeb/agencyName.php",
            dataType: "JSON",
            success: function (data) {
                $("#txtAgenciesCompanyNotify").empty();
                for (var value in data) {
