...
function asignarInstalacion($report,$profileToAssign,$employeeToAssign, $idClient){
    ...
    if ($getEmployeeData->execute()) {
        if ($getEmployeeData->fetch()) {
            $instalation = 4;
              error_log("Si busco el empleado pendiente. ProfileToAssing,Agencia,Tipo de Agencia: ".$profileToAssign.",".$agency.",".$agencyType);
            // if ( ($profileID == 4 || $profileID == 8) && $profileToAssign == 4 ) {
            // SI LA AGENCIA DEL REPORTE ES INSTALADORA SE ASIGNA A EL USUARIO DUMMY INSTALADOR DE LA AGENCIA, SINO SE BUSCA LA AGENCIA INSTALADORA CORRESPONDIENTE DEL MUNICIPIO
                                ...
                                if ($profileToAssign == 3) {
                                    ...
                                    if ($estatusCrontratoReport = $connSegVta->prepare($stmtTEstatus)) {
                                        ...
                                        if ($estatusCrontratoReport->execute()) {
                                            $selecteportTVTASQL = "SELECT id FROM reportTiempoVentas WHERE idReporte = ? and fechaSegundaCaptura != null;";
