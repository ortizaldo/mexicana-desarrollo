        ...
        if ($getFormSell->fetch()) 
        {
            ...
            if ($getFormSellValidation->execute()) 
            {
                ...
                if ($getFormSellValidation->fetch()) 
                 {
                    grabarLog("entro en if donde actualiza");
                     ...
                     if($validateStatus == 0)
                     {
                        ...
                        $imagesStatus->bind_param("iiiiiiii", $trustedHome, $requestImage, $privacyAdvice, $identificationImage, $payerImage, $agreegmentImage, $validacion, $idFormValidation);
                        $imagesStatus->execute();
                         ...
                         {   
                             ...
                             $getHistory->bind_param("ii", $idReport, $idReportType);
                             $getHistory->execute();
                             ...
                             {
                                  // SI HAY ACTUALIZA
                                 $sqlHistory = "UPDATE reportHistory SET  idUserAssigned = ?, idStatusReport = ?,rechazado=1, updated_at = NOW() WHERE idReportHistory = ?;";
                     ...
                     else
                     {
                         // SI VALIDACION ES == 1 
                        $idValidacion = 0;
                        $smtValidacion = $conn->prepare("SELECT id FROM form_sells_validation WHERE idFormSell = ?;");
                        $smtValidacion->bind_param("i", $formNumber);
                        $smtValidacion->execute();
                        $smtValidacion->store_result();
                        $smtValidacion->bind_result($idValidacion);


                        if($smtValidacion->fetch())
                        {
                            $imagesStatus = $conn->prepare("UPDATE form_sells_validation SET trustedHome = ?, requestImage = ?, privacyAdvice = ?, identificationImage = ?, payerImage = ?, agreegmentImage = ?, validate = ?, updated_at = NOW() WHERE id = ?;");
                            $imagesStatus->bind_param("iiiiiiii", $trustedHome, $requestImage, $privacyAdvice, $identificationImage, $payerImage, $agreegmentImage, $validacion, $idFormValidation);
                            $imagesStatus->execute();
                 ...
                 else
                 {
                     ...
                     if($validacion == 0)
                     {
                        ...
                        {  
                            ...
                            /* AGREGADO PARA LA PARTE DE PROCESOS EN PARALELO */
                            $idReportHistory = NULL;
                            $sqlHistory = "SELECT idReportHistory FROM reportHistory AS rh WHERE idReport = ? AND idReportType = ?;";
                            $getHistory = $conn->prepare($sqlHistory);
                            $getHistory->bind_param("ii", $idReport, $idReportType);
                            $getHistory->execute();
