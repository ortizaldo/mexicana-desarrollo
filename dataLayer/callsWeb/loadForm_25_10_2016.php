...
function GetForm($conn, $type, $idForm, $idUsuario)
{
    ...
    $stmtGetForm = $conn->prepare("call spGetformTest(?,?,?);");
    ...
    $stmtGetForm->store_result();
        ...
        if ($type == "Venta") {
            ...
            $stmtGetForm->bind_result($id, $prospect, $uninteresting, $motivosDesinteres, $comments, $owner, $id, $name, $lastName, $lastNameOp, $payment, $financialService, $requestNumber, $meeting, $content, $imageName, $estatus, $puedeValidar,$created_at,$agenciaVendedor, $estatusAsignacionInstalacion);
            ...
            $conn = $DB->getConnect();
            $querySmtFrmPlumb = "SELECT FP.id, FP.consecutive,FP.name,FP.lastName, FP.request,
                                        FP.phLabel, FP.agencyPh, FP.agencyNumber,FP.installation,FP.abnormalities, FP.comments, FP.brand,
                                        FP.type, FP.serialNumber, FP.measurement,FP.latitude,FP.longitude,FP.created_at
                                             FROM report AS RP
                                             INNER JOIN report_employee_form AS REF ON RP.id = REF.idReport
                                             INNER JOIN form_installation AS FP ON FP.id = REF.idForm
                                             WHERE REF.idReport =?";
            //var_dump($conn->prepare($querySmtFrmPlumb));
            if ($stmtFrmPlumb = $conn->prepare($querySmtFrmPlumb)) {
                $stmtFrmPlumb->bind_param("i", $idForm);
                if($stmtFrmPlumb->execute()){
                    ...
                    $stmtFrmPlumb->bind_result($id,$consecutive, $name,$lastName, $request, $phLabel,$agencyPh,$agencyNumber,$installation,
                        $abnormalities,$comments, $brand,$type, $serialNumber,$measurement,$latitude,$longitude,$created_at, $estatusAsignacionInstalacion);
                    if($stmtFrmPlumb->fetch()){
                        $responseArray['id'] = $id;
                        $responseArray['consecutive'] = $consecutive;
                        $responseArray['name'] = $name;
                        ...
                        $responseArray['created_at'] = $created_at;
                        $responseArray["idClienteGenerado"] = $estatusAsignacionInstalacion;
            ...
            $DB = new DAO();
            $conn = $DB->getConnect();
            $querySmtFrmSecondSell = "SELECT rep.id as repCons, rep.agreementNumber, frmVUno.name as nomCliente, 
                                    ...
                                    from report rep, form_sells frmVUno, reportHistory reph
                                    where 0=0
                                    and reph.idFormSell=frmVUno.id 
                                    and reph.idReport=rep.id
            ...
            if ($stmtFrmSecondSell = $conn->prepare($querySmtFrmSecondSell)) {
                ...
                if($stmtFrmSecondSell->execute()){
                    ...
                    $stmtFrmSecondSell->bind_result($repCons,$agreementNumber,$nomCliente,$apPat,$apMat,
                                               $colonia,$street,$betweenStreets,$outterNumber,$idCountry,$idState,
                                               $idCity,$idFormulario,$payment,$financialService,$fechaSol, $idUserAssigned
                    );
                    //var_dump($stmtFrmSecondSell);
                    if($stmtFrmSecondSell->fetch()){
                        $responseArray['repCons']=$repCons;
