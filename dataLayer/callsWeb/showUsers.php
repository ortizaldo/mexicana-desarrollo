...
function retrieveAdmins($conn) {
	...
	/*$getAdmins = "SELECT US.id, US.nickname, US.name, US.lastName, US.email, US.avatar, US.phoneNumber, US.avatar, PF.name
					FROM user AS US INNER JOIN employee AS EM ON EM.id = US.id INNER JOIN agency_employee AS AGEM ON AGEM.idemployee = EM.id
					INNER JOIN agency AS AG ON AG.idUser = AGEM.idAgency INNER JOIN profile AS PF ON PF.id = EM.idProfile
	...
	$getAgenciesResult = "SELECT US.id, US.nickname, US.name, US.lastName, US.lastNameOP, US.email, US.avatar, US.phoneNumber, US.created_at, AG.tipo, AG.plazo FROM agency AS AG INNER JOIN user AS US ON US.id = AG.idUser WHERE US.active=1;";
	...
	$getEmployeesFinal = "   SELECT id, agency, employee, employeeName, employeeLastName, email, phone, profile FROM employeesAssigned
                                            WHERE profile!='Agency' AND profile!='Admin'
                                            ORDER BY id;";
...
function retrieveAgencies($conn) {
	...
	$getAgencies = "SELECT US.id, US.nickname, US.name, US.lastName, US.email, US.avatar, US.phoneNumber, US.photoUrl, rol.type, USAG.name FROM user AS US INNER JOIN user_rol AS USR ON US.id = USR.idUser INNER JOIN rol ON rol.id = USR.idRol INNER JOIN employee AS EM ON EM.idUser = US.id INNER JOIN agency_employee AS AE ON AE.idemployee = EM.id INNER JOIN agency AS AG ON AG.id = AE.idAgency INNER JOIN user AS USAG ON USAG.id = AG.idUser WHERE US.active = 1 AND rol.type = 'Agency';";
	...
	/*$getEmployees = "SELECT US.id, US.nickname, US.name, US.lastName, US.email, US.avatar, US.phoneNumber, US.photoUrl, rol.type, USAG.nickname
                                    FROM user AS US INNER JOIN user_rol AS USR ON US.id = USR.id INNER JOIN rol ON rol.id = USR.idRol
                                    INNER JOIN employee AS EM ON EM.idUser = US.id INNER JOIN agency_employee AS AE ON AE.idemployee = EM.id
                                    INNER JOIN agency AS AG ON AG.id = AE.idAgency INNER JOIN user AS USAG ON USAG.id = AG.idUser
...
function retrieveEmployeesAgency($conn, $agencyID) {
	...
	$getEmployeesByAgency = $conn->prepare("SELECT US.id AS id, US.nickname AS employee, US.name AS employeeName, US.lastName AS employeeLastName
										FROM user AS US INNER JOIN employee AS EM ON EM.idUser = US.id INNER JOIN agency_employee AS AGEM ON AGEM.idemployee = EM.id
										INNER JOIN agency AS AG ON AG.id = AGEM.idAgency INNER JOIN user AS USAG ON USAG.id = AG.idUser
										INNER JOIN profile AS PF ON PF.id = EM.idProfile WHERE US.id <> AGEM.idAgency AND AG.id = ? ORDER BY US.nickname DESC;");
