			...
			if ($createSecondStepSell = $conn->prepare($createSecondStepSellSQL)) {
				...
				if ($createSecondStepSell->execute()) {
					...
					if ($createAgreeReport->execute()) {
						...
						$idRpHist=getIdReportHistSegVta($idReport);
						if ($idRpHist != '') {
							...
							$stmtUpdateFirst="UPDATE reportHistory SET idReport=?,
																	   idStatusReport=?,
																	   updated_at=NOW(),
												WHERE idReportHistory=? AND idReportType=5;";
				            if ($insertHistory = $conn->prepare($stmtUpdateFirst)) {
				            	$insertHistory->bind_param("iii",$solicitud["idAsignacion"], $estatusWorkFlow, $idRpHist);
				                $insertHistory->execute();
				            }else{
				            	error_log($insertHistory->error);
				            }
						}
			...
			if ($stmt = $conn->prepare($createInstallationFormSQL)) {
				...
				if ($stmt->execute()) {
					...
					if ($searchReports->execute()) {
						...
						$updateEstatusContrato = $conn->prepare("UPDATE tEstatusContrato SET `estatusVenta` = ?, `estatusAsignacionInstalacion` = ?, `fechaMod` = NOW() WHERE `idReporte` = ?;");
						$updateEstatusContrato->bind_param("iii",$idStatus, $idStatusIns, $reportID);
						$updateEstatusContrato->execute();
	...
	if ($stmtinsertarFormularioTipoVentaUno = $conn->prepare($stmtinsertarFormularioTipoVentaUnoSQL)) {
		...
		if ($stmtinsertarFormularioTipoVentaUno->execute()) {
			...
			if ($stmtinsertarFormularioTipoVentaUno->fetch()) {
                               ...
                               // $reportHistory = getReportHistory($idReporteGenerado);
                                validarVentaHistory($idReporteGenerado,$idFormSellsGenerado, $IDUser, $estatusWorkFlow, $idSolicitudMovil, $inIdFormulario);
                                //actualizarReportTiempos($idReporteGenerado, $estatusWorkFlow, $reportHistory);
...
/**
         * Funcion que recupera el reportHistory
         */
        function getReportHistory($idReporteGenerado)
        {
            $DB = new DAO();
            $conn = $DB->getConnect();
            ...
            $result = NULL;
            $report = array();
            $historyVenta = "SELECT id,idStatusReport, created_at, updated_at FROM reportHistory WHERE idReporte = $idReporteGenerado AND idReportType = $idReportType;";
            $result = $conn->query($historyVenta);
            ...
            if($result->num_rows > 0)
            {
                ...
                {
                    $report["id"] = $row[0];
                    $report["idStatusReport"] = $row[1];
                    $report["created_at"] = $row[2];
                    $report["updated_at"] = $row[3];
                }
            }
...
/**
         ...
         */
        function actualizarReportTiempos($idReporteGenerado, $estatusWorkFlow, $reportHistory)
        { 
            $DB = new DAO();
            $conn = $DB->getConnect();
            ...
            if(isset($reportHistory["id"]))
            {
                if($reportHistory["idStatusReport"] == $ESTATUS_VENTA_RECHAZADO && $estatusWorkFlow == $ESTATUS_VENTA_COMPLETA)
                {
                    // CUANDO FUE UN RECHAZO Y AL FINAL FUE COMPLETADA
                    $report = getReportHistory($idReporteGenerado);
                    $reportActTiempoSQL = "UPDATE reportTiempoVentas SET fechaFinRechazo = ? WHERE idReporte = ?;";
                    $smtReportActTiempo = $conn->prepare($reportActTiempoSQL);
                    $smtReportActTiempo->bind_param("is",$idReporteGenerado, $report["update_at"]);
                }
                else
                {
                    // CUANDDO NO ES UN RECHAZO Y SOLO QUEDO  COMO PENDIENTE DESDE MOVIL Y SE COMPLETO
            ...
            {
                error_log(array("status" => "entra en  el else", "estatusWorkflow" => $estatusWorkFlow, "idReporteGenerado" => $idReporteGenerado, "reportHistory" => $reportHistory));
                ...
                if($estatusWorkFlow == $ESTATUS_VENTA_COMPLETA)
                {
                    ...
                    $smtReportActTiempo = $conn->prepare($reportActTiempoSQL);
                    $smtReportActTiempo->bind_param("is",$idReporteGenerado, $report["created_at"]);
                }
            }
