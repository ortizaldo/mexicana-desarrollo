		...
		if ($tipoDeFormulario == $FORMULARIO_TIPO_CENSO) {
		   ...
		   // $niple = $formularioArray["id"];
			...
			if ($createSecondStepSell = $conn->prepare($createSecondStepSellSQL)) {
				...
				$createSecondStepSell->bind_param("idisssssssssssssiiisisssddidssssssdd", 
...
//        grabarLog("entraaqui");
	if ($stmtinsertarFormularioTipoVentaUno->execute()) {
		...
		if ($stmtinsertarFormularioTipoVentaUno->fetch()) {
                ...
                validarVentaHistory($idReporteGenerado,$idFormSellsGenerado, $idUsuario, $estatusWorkFlow);
			...
			$responseArray["ReporteGenerado"] = $idReporteGenerado;
			$responseArray["RespuestaActPlom"] = validaEmpleadoTipoPlomero($idUsuario, $idReporteGenerado, $inAgreementNumber, $inIdFormulario, $idFormSellsGenerado, $idSolicitudMovil);
...
function validaEmpleadoTipoPlomero($userID, $idReporte, $inAgreementNumber, $idFormulario,$idFormSellsGenerado, $idSolicitudMovil)
{
	$DB = new DAO();
	$conn = $DB->getConnect();
	if ($userID != "" && $idReporte != "") {
...
function insertarRelacionFotografiasDeLaVenta($conn, $idFormSellsGenerado, $idFotografia,$key,$sizeArrayFotografias)
{
	...
	function grabarLog($logInfo)
	{
        ...
        function validarVentaHistory($idReporteGenerado,$idFormSellsGenerado, $idUsuario, $estatusWorkFlow )
        {
                $DB = new DAO();
                $conn = $DB->getConnect();
...
//                        grabarLog("entro en la funcion llego if donde si habia uno anterior y tiene que actualziar");
                        $stmtUpdate="UPDATE reportHistory SET  idUserAssigned = ?, idStatusReport = ?, updated_at = NOW() WHERE idReportHistory = ?;";
                        if ($update = $conn->prepare($stmtUpdate)) 
                        {
                            $update->bind_param("iii", $idUsuario, $estatusWorkFlow,  $idReportHistory);
                            $update->execute();
                        }
                    }
                    else
...
//                        grabarLog("entro en la funcion llego else donde no hay registro de venta y tiene que insertar");
                        $stmtInsertFirst="INSERT INTO `reportHistory`(`idReport`,`idFormSell`,`idReportType`,`idUserAssigned`,`idStatusReport`,`updated_at`,`created_at`)VALUES(?,?,?,?,?,NOW(),NOW());";
                        if ($insertHistory = $conn->prepare($stmtInsertFirst)) 
                        {
                            $insertHistory->bind_param("iiiii",$idReporteGenerado, $idFormSellsGenerado, $valor, $idUsuario, $estatusWorkFlow);
